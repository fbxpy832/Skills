...
if command -v oma >/dev/null 2>&1 || command -v oh-my-openagent >/dev/null 2>&1 || command -v openagent >/dev/null 2>&1; then oma_available=true; fi
if grep -Eqi "$keywords" "$OMA_MD"; then oma_available=true; fi
...
if [[ "$opencode_available" == true ]]; then
  ...
  set +e
  run_limited 90 opencode run "$prompt" > "$TRIGGER_MD.tmp" 2>&1
  rc=$?
  set -e
  {
  ...
  rm -f "$TRIGGER_MD.tmp"
  if [[ "$rc" -eq 0 ]] && grep -Eqi 'oma|oh.my.openagent|openagent|agent|model|provider' "$TRIGGER_MD"; then
