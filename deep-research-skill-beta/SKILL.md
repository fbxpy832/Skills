...
- 多个模型来源：DeepSeek、Moonshot/Kimi、OpenRouter、SiliconFlow 或自定义 OpenAI-compatible endpoint；可一次配置多个 provider。
...
- DeepSeek V4 Flash（DeepSeek 官方 API）：当前所有 DeepSeek 角色临时统一使用，包括规划、搜索、分析、测算、写作和审计。
- DeepSeek V4 Flash（DeepSeek 官方 API）：长文档阅读、多文件摘要、政策/合同/招投标/财报/技术文档归纳也统一使用。

当前临时策略：

- 不再使用 OpenCode Go 的 `opencode-go/*` 模型。
- DeepSeek 官方 API 全部使用 V4 Flash，直到后续重新放开 Pro 路由。
- 长文档角色也默认使用 DeepSeek V4 Flash，不启用 OpenAI 模型。
- 因当前审计也使用 Flash，关键决策结论必须保留人工复核项，不得只凭 Flash 输出直接作为正式结论。

禁止：

- 恢复 `opencode-go/*` 模型路由。
- 把 DeepSeek Flash 的“请求模型”写成“已验证实际使用模型”。
- 在无人工复核的情况下，把仅由 Flash 审计通过的正式决策报告标记为“正式版”。
