...
```text
研究主题-报告类型[-来源类型][-离线初稿|-待联网核验版][-v2]-YYYY-MM-DD.md
```

示例：
- `智慧停车商业模式-公司经营决策研究报告-2026-05-30.md`
- `端侧AI技术路线-技术路线研究报告-待联网核验版-2026-05-30.md`
- `某公司投资价值分析-投资分析报告-多源版-v2-2026-05-30.md`

正文 Markdown 的一级标题应替换为：
```text
# 研究主题：报告类型
```
例如：
...
- 不覆盖原文件，除非用户明确要求。
- 如果目标文件已存在，追加 `-v2`、`-v3`（版本号放在日期前）。
- 二次优化加入"优化版""立项版""审校版"等后缀。
- 外部搜索失败、来源抓取失败或来源不足时，文件名必须加入 `-离线初稿` 或 `-待联网核验版`。
- 当报告仅依赖单一来源类型（如仅本地 Vault、仅模型推理）时，文件名应体现来源类型组成，例如 `-Vault版`、`-模型推理版`、`-多源版`，以便快速识别报告的来源结构。
- 文件命名中，来源类型后缀和降级后缀放在研究主题和报告类型之后、日期之前。
- 文件名去除 `/\:*?"<>|` 等不安全字符。
...
来源使用情况: [列出使用的来源类型，如 external_authoritative / external_media / local_vault / local_wiki / uploaded_files / model_reasoning]
...
- 宿主状态栏、命令输出、运行日志或 API 响应可检测实际模型：写"检测到实际模型为 ..."。
- 无法检测：写"模型使用未能自动验证"。
