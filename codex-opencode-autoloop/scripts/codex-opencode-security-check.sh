...
deleted_count="$( (git diff --name-status -- . ':(exclude).codex-opencode/*' 2>/dev/null || true) | awk '$1=="D"{c++} END{print c+0}' )"
...
if (git diff --shortstat -- . ':(exclude).codex-opencode/*' 2>/dev/null || true) | grep -Eq '([5-9][0-9]{3,}|[1-9][0-9]{4,}) (insertions|deletions)'; then
