...
mkdir -p "$STATUS" "$LOGS" "$PROMPTS" "$REPORTS" "$RUNS" "$TASKS"
...
REQUIRE_TESTS_PASS="$(json_get review_policy.require_tests_pass true)"

RUN_ID="$(date '+%Y%m%d-%H%M%S')"
RUN_DIR="$RUNS/$RUN_ID"
mkdir -p "$RUN_DIR"
TASK_RECORD="$TASKS/$RUN_ID-task.md"
SUMMARY_FILE="$RUN_DIR/summary.md"
REPLY_RECORD="$STATUS/last-codex-reply.json"
...
record_reply() {
  local command="$1" exit_code="$2" test_result="$3"
  python3 - "$REPLY_RECORD" "$command" "$exit_code" "$RUN_ID" "$STATUS/live.md" "$SUMMARY_FILE" "$test_result" <<'PY'
import json, sys, datetime
path, command, exit_code, run_id, status_file, summary_file, test_result = sys.argv[1:]
data = {
...
write_summary() {
  local round="$1" status="$2" result="$3" command="$4" exit_code="$5" test_result="$6" security_result="$7" next_action="$8" errors="$9" risk_level="${10}"
  local files diffstat tests_run
  files="$(git diff --name-only -- . ':(exclude).codex-opencode/*' 2>/dev/null | sanitize || true)"
  diffstat="$(git diff --stat -- . ':(exclude).codex-opencode/*' 2>/dev/null | sanitize || true)"
  if [[ -n "$TEST_COMMAND" ]]; then
    tests_run="$TEST_COMMAND ($test_result)"
  else
    tests_run="not detected ($test_result)"
  fi
  cat > "$SUMMARY_FILE" <<EOF
# AutoLoop Compact Summary
...
\`\`\`
EOF
  record_reply "$command" "$exit_code" "$test_result"
}
...
run_opencode() {
  ...
  LAST_OPENCODE_COMMAND="$OPENCODE_COMMAND ${OPENCODE_ARGS[*]} < $prompt_file"
...
set +e
bash "$BASE/scripts/codex-opencode-preflight.sh" > "$LOGS/preflight-run.log" 2>&1
preflight_rc=$?
set -e
if [[ "$preflight_rc" -ne 0 ]]; then
  OMA_AVAILABLE="unknown"
  update_state failed 0 preflight "Preflight failed." "Fix preflight errors before starting OpenCode/OMA." "preflight failed"
  write_summary 0 failed failed "bash .codex-opencode/scripts/codex-opencode-preflight.sh" "$preflight_rc" unknown unknown "Fix preflight and rerun." "Preflight failed. See .codex-opencode/logs/preflight-run.log." high
  finish_report failed 0 "Preflight failed." "bash .codex-opencode/scripts/codex-opencode-preflight.sh"
  exit 2
fi
...
if [[ "$OMA_MODE" == "forced_oma" && "$OMA_AVAILABLE" != "available" ]]; then
  ...
  write_summary 0 failed failed "bash .codex-opencode/scripts/codex-opencode-preflight.sh" 0 unknown unknown "Run preflight and fix OMA/OpenCode configuration." "forced_oma requested but OMA unavailable." high
...
if [[ "$OMA_MODE" == "auto" && "$OMA_AVAILABLE" != "available" && "$FALLBACK" != "true" ]]; then
  ...
  write_summary 0 failed failed "bash .codex-opencode/scripts/codex-opencode-preflight.sh" 0 unknown unknown "Enable fallback or fix OMA." "OMA unavailable and fallback disabled." high
...
if [[ "$OPENCODE_AVAILABLE" != "available" && "$DRY_RUN" -ne 1 ]]; then
  ...
  write_summary 0 failed failed "command -v $OPENCODE_COMMAND" 127 unknown unknown "Install or expose OpenCode in PATH." "OpenCode CLI not found." high
  finish_report failed 0 "OpenCode CLI not found." "bash .codex-opencode/scripts/codex-opencode-preflight.sh"
  exit 127
fi

printf '%s\n' "$TASK" > "$TASK_RECORD"
cp "$TASK_RECORD" "$RUN_DIR/task.md"
{
...
write_summary 0 running initialized "bash .codex-opencode/scripts/codex-opencode-preflight.sh" 0 unknown unknown "Start round 1 via OpenCode/OMA." None low

final_result="failed"
rounds_done=0
remaining="Max rounds reached or review failed."
last_test_status="unknown"
last_security_status="unknown"
...
EOF
  ...
  fi
  {
    echo
	    ...
	    echo "Write a compact summary with files_changed, tests_run, errors, next_action, and risk_level."
	  } >> "$prompt_file"
  ...
  if ! run_opencode "$prompt_file" "$output_file"; then
    ...
    write_summary "$round" failed failed "$LAST_OPENCODE_COMMAND" "$LAST_OPENCODE_EXIT_CODE" unknown unknown "Fix CLI/auth/model issue and rerun preflight." "OpenCode invocation failed." high
  ...
  if [[ "$sec_rc" -ne 0 ]]; then
    write_summary "$round" needs_user_action failed "bash .codex-opencode/scripts/codex-opencode-security-check.sh" "$sec_rc" "$test_status" needs_user_action "Review security log before continuing." "Security check requires user action." high
    finish_report needs_user_action "$round" "Security check requires user action." "Review .codex-opencode/logs/security.log"
    exit 4
  fi
	  security_status="passed"
	  last_security_status="$security_status"
  ...
  if [[ "$test_status" == "skipped" && "$REQUIRE_TESTS_PASS" == "true" ]]; then
    result="FAIL"; problems+="Tests were skipped while require_tests_pass is true. "; required+="Add or configure a test command and rerun. "
  fi
...
EOF
  ...
  if [[ "$result" == "PASS" ]]; then
    ...
    remaining="$([[ "$test_status" == "skipped" ]] && echo 'Accepted only because require_tests_pass is false; no automated test command was detected.' || echo 'No remaining blocking issue detected.')"
    update_state completed "$round" completed "Review passed." "Inspect final report." None reviewer "$test_status" PASS "$security_status"
    write_summary "$round" completed passed "$LAST_OPENCODE_COMMAND" "$LAST_OPENCODE_EXIT_CODE" "$test_status" "$security_status" "Inspect final report." None low
...
EOF
  ...
  write_summary "$round" repairing failed "$LAST_OPENCODE_COMMAND" "$LAST_OPENCODE_EXIT_CODE" "$test_status" "$security_status" "Run next OpenCode/OMA repair round." "$problems" medium
done

update_state failed "$rounds_done" completed "Max rounds reached without PASS." "Inspect final report and latest repair prompt." "max rounds reached" reviewer "$last_test_status" FAIL "$last_security_status"
write_summary "$rounds_done" failed failed "${LAST_OPENCODE_COMMAND:-unknown}" "${LAST_OPENCODE_EXIT_CODE:-1}" "$last_test_status" "$last_security_status" "Inspect latest repair prompt and rerun autoloop." "Max rounds reached without PASS." medium
