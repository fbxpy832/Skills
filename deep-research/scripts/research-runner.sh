...
# Deep Research CLI Runner
# 运行研究子任务，按 mode + agent 调用 model-router.sh
#
# 用法: research-runner.sh MODE TASK_FILE [OUTPUT_DIR] [PROJECT_DIR] [--dry-run] [--parallel|--sequential]
#
# 依赖: research-runner.sh 依赖 CLI 工具执行。设置 CLI_BIN 环境变量指定使用的 CLI。
#       默认读取 $CLI_BIN 环境变量，如未设置会尝试检测常见 CLI。
...
PROJECT_DIR="${4:-$(pwd)}"
DRY_RUN="${DRY_RUN:-0}"
EXECUTION_MODE="${DEEP_RESEARCH_EXECUTION_MODE:-sequential}"
...
if [ -z "$TASK_FILE" ]; then
  ...
  echo "Usage: research-runner.sh MODE TASK_FILE [OUTPUT_DIR] [PROJECT_DIR] [--dry-run] [--parallel|--sequential]"
  echo "Example: research-runner.sh high_quality /tmp/task.md /tmp/research-run /path/to/project --parallel"
  exit 1
fi
...
if [ "$DRY_RUN" != "1" ] && { [ -z "$CLI_BIN" ] || [ ! -x "$CLI_BIN" ]; }; then
  echo "WARNING: No CLI tool found. Set CLI_BIN or install a supported CLI."
  echo "The runner will generate prompts but cannot execute agents."
  DRY_RUN=1
fi
...
7. Treat the requested capability as a suggested capability only. Do not write that a specific model was actually used unless execution logs or CLI state explicitly verify it.
...
run_agent() {
  ...
  local capability prompt_file output_file deps_text deps_display start_time end_time
  local -a deps=()
  capability="$("$ROUTER" "$MODE" "$agent")"
  ...
  write_prompt "$agent" "$capability" "$prompt_file" "$deps_text"

  echo "Prepared $agent with capability $capability"
  ...
  if [ "$DRY_RUN" = "1" ]; then
    {
      ...
      echo "- Requested capability: $capability"
    ...
    if [ -z "$CLI_BIN" ] || [ ! -x "$CLI_BIN" ]; then
      echo "ERROR: CLI tool not found. Set CLI_BIN or install a supported CLI."
      return 1
    fi

    if [ "$EXECUTION_MODE" = "parallel" ]; then
      echo "WARNING: Parallel execution not supported, falling back to sequential mode"
      EXECUTION_MODE="sequential"
      if ! (
        cd "$PROJECT_DIR"
        if [ -n "${CLI_RUN_TEMPLATE:-}" ]; then
          eval "$(echo "$CLI_RUN_TEMPLATE" | sed "s|\${CLI_BIN}|$CLI_BIN|g; s|\${MODEL}|$capability|g; s|\${PROMPT}|$(cat "$prompt_file")|g")"
        else
  ...
  {
    echo "agent=$agent"
    echo "requested_capability=$capability"
...
- model_statement_rule: Do not claim a requested capability was actually used unless CLI logs, command output, or UI state verifies it.
...
for agent in planner_agent source_agent long_context_agent analyst_agent scenario_agent writer_agent reviewer_agent; do
  if agent_enabled "$agent"; then
    requested_capability="$( "$ROUTER" "$MODE" "$agent" )"
    echo "- $agent: requested_capability=$requested_capability; actual_model_detection_status=not_verified" >> "$summary_file"
    echo "- $agent: requested_capability=$requested_capability; actual_model_detection_status=not_verified" >> "$execution_context_file"
  fi
done
...
- reporting_rule: Requested capabilities must not be described as verified actual model usage.
EOF

echo ""
echo "Deep Research run complete."
