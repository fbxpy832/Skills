...
- 默认必须发起外部联网搜索（webfetch / web search / API 检索），不可跳过。
- 仅当用户**明确**指示"不需要联网搜索""离线分析""不用搜索""仅本地资料"时，方可跳过外部搜索。
- 用户说"快速初稿""先给框架""简单整理"等不构成跳过搜索的理由——除非明确说"不用联网"。
- 外部搜索失败的，必须记录 `source_failure_log`，按降级规则标注为"离线初稿"或"待联网核验版"，不得标为"正式高质量报告"。
- `high_quality` 模式下搜索失败 → 审计等级最高 `CONDITIONAL_PASS`。
- `cost_saving` / `draft_fast` 模式下，仍需尝试外部搜索；搜索失败时不强制降级但必须记录。

**搜索工具方案**：见 [references/search-tools.md](references/search-tools.md)。搜索顺序：先 Brave Search API（Scheme A）→ 结果不足则 Bing 补充（Scheme B/D）→ 仍不足则直接抓取已知来源（Scheme C）。禁止跳过 Brave API 直接降级。搜索执行后必须报告"用了哪些 Scheme + 成功/失败 + 下一步"。
