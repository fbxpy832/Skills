...
provider_defaults() {
  case "$1" in
    ...
    moonshot)
      ...
      ;;
    openai)
      echo "openai|OpenAI|https://api.openai.com/v1|OPENAI_API_KEY|gpt-5.2|gpt-5.4-mini|gpt-5.2"
      ;;
    openrouter)
...
choose_template() {
  ...
  echo "  4) OpenAI" >&2
  echo "  5) OpenRouter" >&2
  echo "  6) SiliconFlow" >&2
  echo "  7) Custom OpenAI-compatible" >&2
  local choice
...
prompt_agent_route() {
  ...
  provider="$(prompt "$agent provider id" "$default_provider")"
