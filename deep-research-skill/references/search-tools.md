...
### Brave Search API（通用 fallback，已集成到 search.sh）
...
" 2>/dev/null)
    ...
    # 缓存未命中：执行搜索
result=$(export https_proxy=http://127.0.0.1:7890
export http_proxy=http://127.0.0.1:7890
...
### 搜索尝试顺序
   - **第 1 步**：Scheme A（统一搜索脚本）— 根据 `search.sh` 语言路由和已配置 key 自动选择 Bocha / Exa / Brave
